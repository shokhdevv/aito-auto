        ...
        <div ref={contentRef} onClick={e => e.stopPropagation()} className="w-full max-w-[460px] flex flex-col text-white items-center gap-3.5 text-center md:gap-5 rounded-lg p-6 md:p-9 bg-[#050505]"> 
          ...
          <form className="w-full flex flex-col gap-4 ">
            <div className="space-y-2">
              <input  id='name' type="text" placeholder={t('input.reason')} className='w-full focus:outline-none px-3 py-2 !leading-[1.2] lg:px-4 lg:py-3 text-white font-futura max-lg:text-sm rounded bg-currentDark border-[0.5px] border-[#FFFFFFE5]' />
              {'errors.name' && (
                <p className="error-text text-red-600 text-xs lg:text-sm text-start">Укажите свое имя</p>
              )}
            </div>
            <div className="space-y-2">
              <div className="relative w-full">
                ...
                >
                  <option value=" ">{t('input.model')}</option>
                  {/* Other options */}
                </select>
            ...
            <ButtonUI type={'submit'} text={t('btn.send')} extraStyle={'w-fit self-center'}/>
          </form>
        </div>
      </div>
    </div>
